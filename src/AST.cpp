...
// IdNode
IdNode::IdNode(const std::string& i) : name(i) {
    type = ValueType::UNKNOWN;
}
...
GeneratorNode::GeneratorNode(std::shared_ptr<IdNode> i, std::shared_ptr<ExprNode> d, std::shared_ptr<ExprNode> b)
    : id(std::move(i)), domain(std::move(d)), body(std::move(b)) {
    type = ValueType::VECTOR;
}
...
FilterNode::FilterNode(std::shared_ptr<IdNode> i, std::shared_ptr<ExprNode> d, std::shared_ptr<ExprNode> p)
    : id(std::move(i)), domain(std::move(d)), predicate(std::move(p)) {
    type = ValueType::VECTOR;
}
...
VectorDecNode::VectorDecNode(std::shared_ptr<IdNode> i, std::shared_ptr<ExprNode> v)
    : id(std::move(i)), vectorValue(std::move(v)) {
    type = ValueType::VECTOR;
}
...
CondNode::CondNode(std::shared_ptr<ExprNode> cond, std::vector<std::shared_ptr<ASTNode>> stmts)
    : ifCond(std::move(cond)), body(std::move(stmts)) {
    type = ValueType::UNKNOWN;
}
...
LoopNode::LoopNode(std::shared_ptr<ExprNode> cond, std::vector<std::shared_ptr<ASTNode>> stmts)
    : loopCond(std::move(cond)), body(std::move(stmts)) {
    type = ValueType::UNKNOWN;
}
